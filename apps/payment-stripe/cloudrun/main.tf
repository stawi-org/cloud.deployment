# payment-stripe — integration rail; shared topics owned by payment-payment.
...
locals {
  ...
  prompts_topic      = "payment-stripe-prompts"
  payments_topic     = "payment-stripe-payments"
  prompts_ref        = "stripe-prompts"
  payments_ref       = "stripe-payments"
...
module "frame" {
  ...
  oauth2_service_client_id = "service-payment-stripe"

  service_env_extra = {
    QUEUE_STRIPE_PROMPT_NAME  = local.prompts_ref
    QUEUE_STRIPE_PROMPT_URI   = "push://${local.prompts_ref}?protocol=gcppubsub"
    QUEUE_STRIPE_PAYMENT_NAME = local.payments_ref
    QUEUE_STRIPE_PAYMENT_URI  = "push://${local.payments_ref}?protocol=gcppubsub"
  }
