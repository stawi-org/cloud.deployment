# payment-airtel — integration rail; shared topics owned by payment-payment.
...
locals {
  ...
  prompts_topic      = "payment-airtel-prompts"
  payments_topic     = "payment-airtel-payments"
  prompts_ref        = "airtel-prompts"
  payments_ref       = "airtel-payments"
...
module "frame" {
  ...
  oauth2_service_client_id = "service-payment-airtel"
  # Multi-queue rails: shared OIDC audience = service root (not events path).
  push_oidc_audience = local.push_oidc_audience

  service_env_extra = {
    QUEUE_AIRTEL_PROMPT_NAME  = local.prompts_ref
    QUEUE_AIRTEL_PROMPT_URI   = "push://${local.prompts_ref}?protocol=gcppubsub"
    QUEUE_AIRTEL_PAYMENT_NAME = local.payments_ref
    QUEUE_AIRTEL_PAYMENT_URI  = "push://${local.payments_ref}?protocol=gcppubsub"
  }
