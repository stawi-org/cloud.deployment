# communications-at — Frame Cloud Run via modules/frame-cloudrun-app.
...
locals {
  ...
  send_topic         = "notification-africastalking-send"
  send_ref           = "africastalking-send"
...
module "frame" {
  ...
  oauth2_service_client_id = "service-notification-africastalking"
  ...
  service_env_extra = {
    QUEUE_NOTIFICATION_AFRICASTALKING_DEQUEUE_NAME = local.send_ref
    QUEUE_NOTIFICATION_AFRICASTALKING_DEQUEUE_URI  = "push://${local.send_ref}?protocol=gcppubsub"
  }
