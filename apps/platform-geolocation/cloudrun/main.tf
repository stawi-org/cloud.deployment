# platform-geolocation — platform domain Frame service.
# Parity: service-profile apps/geolocation
...
locals {
  ...
  app_env = {
    ...
    OAUTH2_RESOURCE_AUDIENCE          = "${local.api_base}/geolocation"
    OAUTH2_REQUESTED_AUDIENCES = join(",", [
      "${local.api_base}/profile",
      "${local.api_base}/tenancy",
      "${local.api_base}/geolocation",
    ])
