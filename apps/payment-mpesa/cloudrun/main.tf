# payment-mpesa — integration rail; shared topics owned by payment-payment.
...
locals {
  ...
  prompts_topic      = "payment-mpesa-prompts"
  payments_topic     = "payment-mpesa-payments"
  prompts_ref        = "mpesa-prompts"
  payments_ref       = "mpesa-payments"
...
module "frame" {
  ...
  oauth2_service_client_id = "service-payment-mpesa"

  service_env_extra = {
    QUEUE_MPESA_PROMPT_NAME  = local.prompts_ref
    QUEUE_MPESA_PROMPT_URI   = "push://${local.prompts_ref}?protocol=gcppubsub"
    QUEUE_MPESA_PAYMENT_NAME = local.payments_ref
    QUEUE_MPESA_PAYMENT_URI  = "push://${local.payments_ref}?protocol=gcppubsub"
  }
