# payment-jenga — integration rail; shared topics owned by payment-payment.
...
locals {
  ...
  prompts_topic      = "payment-jenga-prompts"
  payments_topic     = "payment-jenga-payments"
  prompts_ref        = "jenga-prompts"
  payments_ref       = "jenga-payments"
...
module "frame" {
  ...
  oauth2_service_client_id = "service-payment-jenga"
  ...
  extra_secret_ids  = toset([
    "jenga-api-key",
    "jenga-consumer-secret",
    "jenga-merchant-code",
  ])
  extra_version_ids = toset([])
  secret_env_extra = {
    JENGA_API_KEY = { secret = "jenga-api-key" }
    JENGA_CONSUMER_SECRET = { secret = "jenga-consumer-secret" }
    JENGA_MERCHANT_CODE = { secret = "jenga-merchant-code" }
  }

  service_env_extra = {
    QUEUE_JENGA_PROMPT_NAME  = local.prompts_ref
    QUEUE_JENGA_PROMPT_URI   = "push://${local.prompts_ref}?protocol=gcppubsub"
    QUEUE_JENGA_PAYMENT_NAME = local.payments_ref
    QUEUE_JENGA_PAYMENT_URI  = "push://${local.payments_ref}?protocol=gcppubsub"
  }
  ...
  app_env = {
    ...
    JENGA_ENV = "https://uat.finserve.africa"
    JENGA_CALLBACK_URL = "https://api.stawi.org/payment/receivepayments"
  }
}
