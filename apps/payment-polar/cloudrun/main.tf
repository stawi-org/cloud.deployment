# payment-polar — integration rail; shared topics owned by payment-payment.
...
locals {
  ...
  prompts_topic      = "payment-polar-prompts"
  payments_topic     = "payment-polar-payments"
  prompts_ref        = "polar-prompts"
  payments_ref       = "polar-payments"
...
module "frame" {
  ...
  oauth2_service_client_id = "service-payment-polar"

  service_env_extra = {
    QUEUE_POLAR_PROMPT_NAME  = local.prompts_ref
    QUEUE_POLAR_PROMPT_URI   = "push://${local.prompts_ref}?protocol=gcppubsub"
    QUEUE_POLAR_PAYMENT_NAME = local.payments_ref
    QUEUE_POLAR_PAYMENT_URI  = "push://${local.payments_ref}?protocol=gcppubsub"
  }
