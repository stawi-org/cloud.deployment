# platform-files — platform domain Frame service.
# Parity: service-files apps/default
# Auth: identity Hydra/Keto in stawi-identity (cross-project URLs).
...
# Cross-project identity services (OIDC + ReBAC).
...
locals {
  ...
  app_env = {
    ...
    OAUTH2_RESOURCE_AUDIENCE          = "${local.api_base}/files"
    OAUTH2_REQUESTED_AUDIENCES = join(",", [
      "${local.api_base}/profile",
      "${local.api_base}/tenancy",
      "${local.api_base}/files",
    ])
    ...
    APP_NAME                           = var.app_name
    # Storage defaults (override via secrets / extra env later for R2)
    STORAGE_PROVIDER  = "S3"
...
# hydra-webhook-psk is copied into the platform project (same value as identity).
