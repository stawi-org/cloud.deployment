# payment-mtn — integration rail; shared topics owned by payment-payment.
...
locals {
  ...
  prompts_topic      = "payment-mtn-prompts"
  payments_topic     = "payment-mtn-payments"
  prompts_ref        = "mtn-prompts"
  payments_ref       = "mtn-payments"
...
module "frame" {
  ...
  oauth2_service_client_id = "service-payment-mtn"

  service_env_extra = {
    QUEUE_MTN_PROMPT_NAME  = local.prompts_ref
    QUEUE_MTN_PROMPT_URI   = "push://${local.prompts_ref}?protocol=gcppubsub"
    QUEUE_MTN_PAYMENT_NAME = local.payments_ref
    QUEUE_MTN_PAYMENT_URI  = "push://${local.payments_ref}?protocol=gcppubsub"
  }
